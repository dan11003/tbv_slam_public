...
Eigen::Affine3d VectorToAffine3dxyez(const std::vector<double>& vals) {
  if(vals.size() != 3)
  {
    throw std::invalid_argument("VectorToAffine3dxyez: input size error");
  }

  return Eigen::Translation<double, 3>(vals[0],vals[1],0) *
      Eigen::AngleAxis<double>(0, Eigen::Vector3d::UnitX()) *
      Eigen::AngleAxis<double>(0, Eigen::Vector3d::UnitY()) *
      Eigen::AngleAxis<double>(vals[2], Eigen::Vector3d::UnitZ());
}
void VisualizeScanContext(MatrixXd& M, const std::string& window_name, const bool normalize){
  cv::Mat scan_mat_cv(M.rows(), M.cols(), CV_8UC1);
  cv::eigen2cv(M, scan_mat_cv);
  if(normalize)
    cv::normalize(scan_mat_cv, scan_mat_cv, 0, 255, cv::NORM_MINMAX);
  scan_mat_cv.convertTo(scan_mat_cv, CV_8UC1);
  cv::Mat ds_scan_man_color(M.rows(),M.cols(),CV_8UC1);
  cv::applyColorMap(scan_mat_cv, ds_scan_man_color, cv::COLORMAP_JET);

  cv::namedWindow(window_name , cv::WINDOW_NORMAL);
  cv::resizeWindow(window_name , M.rows(),M.cols());
  cv::imshow(window_name, ds_scan_man_color);
}
void SaveScanContext(MatrixXd& M, const std::string& name, const bool normalize){
  cv::Mat scan_mat_cv(M.rows(), M.cols(), CV_8UC1);
  cv::eigen2cv(M, scan_mat_cv);
  if(normalize)
    cv::normalize(scan_mat_cv, scan_mat_cv, 0, 255, cv::NORM_MINMAX);
  ...
  cv::Mat ds_scan_man_color(M.rows(),M.cols(),CV_8UC1);
  cv::applyColorMap(scan_mat_cv, ds_scan_man_color, cv::COLORMAP_JET);
  cv::imwrite(name+".jpg", ds_scan_man_color);
}
...
double SCManager::distDirectSC ( MatrixXd &_sc1, MatrixXd &_sc2 )
{
  ...
  num_eff_cols = std::max(num_eff_cols,1);
...
MatrixXd SCManager::makeScancontext( pcl::PointCloud<SCPointType> & _scan_down )
{
  ...
  for (int pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++)
  {
    ...
    pt.z = _scan_down.points[pt_idx].intensity;
...
void SCManager::makeAndSaveScancontextAndKeys( pcl::PointCloud<SCPointType> & _scan_down )
{
  ...
  std::vector<float> ringkey_std_vek = eig2stdvec( ringkey );

  polarcontexts_.push_back( sc );
  polarcontext_invkeys_.push_back( ringkey );
  polarcontext_invkeys_mat_.push_back( ringkey_std_vek );//
...
std::vector<candidate> SCManager::detectLoopClosureID ( void )
{
  ...
  const double min_dist = 10000000; // init with somthing large
  const int nn_align = 0;
  const int nn_idx = 0;
  const int argmin_shift = 0;
  std::vector<candidate> similar_candidates = {{min_dist, min_dist, 0.0, nn_align, nn_idx, argmin_shift}};
  ...
  for ( int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++ )
  {
    ...
    const double candidate_dist = sc_dist_result.first;
    const float candidate_align = deg2rad(sc_dist_result.second * PC_UNIT_SECTORANGLE);
    const int candidate_shift = sc_dist_result.second;


    similar_candidates.push_back({candidate_dist, candidate_dist, 0.0, candidate_align, (int)candidate_indexes[candidate_iter_idx], candidate_shift});
  ...
  auto itr_end = std::prev(similar_candidates.end()); // AT least one element exists
  for(auto itr = similar_candidates.begin() ; itr != similar_candidates.end() ; itr++)
  {
    if( itr->min_dist < SC_DIST_THRES )
    {
      itr_end = itr;
      cout << "[Loop found] Nearest distance: " << itr->min_dist << " btn " << polarcontexts_.size()-1 << " and " << itr->nn_idx << "." << endl;
      cout << "[Loop found] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " rad." << endl;
    }
    else
    {
      std::cout.precision(3);
      cout << "[Not loop] Nearest distance: " << itr->min_dist << " btn " << polarcontexts_.size()-1 << " and " << itr->nn_idx << "." << endl;
      cout << "[Not loop] yaw diff: " << itr->yaw_diff_rad<< " rad." << endl;
    }
  }
  ...
  auto vek = std::vector<candidate>(similar_candidates.begin(), itr_end);
  cout << "FOUND: " << vek.size() << endl;
  return vek;
